"use client";

import Button from "@/app/(landing)/components/UI/button";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { login } from "@/app/services/auth-services";

const LoginPage = () => {

    const router = useRouter();

    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    const [errorMessage, setErrorMessage] = useState("");
    const [isLoading, setIsLoading] = useState(false);

    useEffect(() => {
        const token = localStorage.getItem("token");

        if (token) {
            router.push("/admin/products");
        }
    }, [router]);

    const handleLogin = async () => {
        setIsLoading(true);

        try {
            const data = await login({
                email,
                password,
            });

            if (data.token) {

                router.push("/admin/products");
            }
        } catch (err: any) {
            setErrorMessage(
                err.message || "Something went wrong, please try later."
            );

            console.error("Login error:", err);
        } finally {
            setIsLoading(false);
        }
    };

    
    return (
        <main className="bg-[#F7F9FA] w-full min-h-screen flex items-center justify-center">
            <div className="max-w-136 w-md bg-white rounded-xl border-t-4 border-primary p-12 py-4">
                <Image src="/images/logo-admin.svg" alt="Logo" width={215} height={36} className="mx-auto mb-4 mt-6"/> 
                <p className="opacity-50 text-sm text-center mb-5">Enter your credentials to access the dashboard</p>
              

                {errorMessage && (
                  <div className="px-3 py-1 bg-primary-light border border border-primary rounded-md text-primary text-sm text-center w-full">
                    {errorMessage}
                </div>
                 )}
                <div className="input-group-admin mb-5">
                    <label htmlFor="email">Email</label>
                    <input type="email" id="email" name="email" 
                    placeholder="Please Type Your Email" 
                    value = {email}
                    onChange = {(e) => setEmail(e.target.value)}/>
                    
                </div>
                <div className="input-group-admin mb-5">
                    <label htmlFor="password">Password</label>
                    <input type="password" 
                    id="password" name="password" 
                    placeholder="••••••••••••••••••••" 
                    value = {password}
                    onChange={(e) => setPassword(e.target.value)}/>
                </div>
                <Button size="small" 
                className="w-full rounded-md  hover:bg-primary/90 mb-8 mt-5" 
                onClick={handleLogin}>
                     {isLoading ? "Signing In..." : "Sign In"}
                </Button>
               
            </div>
        </main>
    );
}
export default LoginPage;